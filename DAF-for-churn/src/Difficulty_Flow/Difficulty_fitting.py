...
class FitModel():
    ...
    def oneuser_fit(self, x, y):
        ...
        y_smooth = np.array([
            (y_cum[min(i + Neighbor,
                       len(y) - 1)] - y_cum[max(i - Neighbor, 0)]) /
            (2 * Neighbor + 1 - max(Neighbor - i, 0) -
             max(Neighbor - (len(y) - i), 0)) for i in range(len(y))
        ])
    ...
    def group_fit(self, win_interactions):
        ...
        if "session_length" in win_interactions.columns and "session_depth" in win_interactions.columns:
            win_interactions = win_interactions.loc[
                win_interactions.session_depth <
                win_interactions.session_length - 1]

        if "level_id" in win_interactions.columns:
            retry_data = win_interactions.groupby(
                ["user_id",
                 "level_id"]).head(1).copy()  # get the first time win
        else:
            retry_data = win_interactions

        new_global_retry = retry_data.groupby(
            ["level_id"]).retry_time.mean().reset_index()
        new_global_retry.rename(columns={"retry_time": "global_retrytime"},
                                inplace=True)
        retry_data = retry_data.merge(new_global_retry,
                                      on=["level_id"],
                                      how="left")
...
def Get_group_ab(filepath="interactions.csv", Neighbor_num=5):
    ...
    win_interactions = Interactions.loc[Interactions.win == 1].reset_index(
        drop=True)
    logging.info("Fitting difficulty curves...")
    model = FitModel(Neighbor_num)
    param_dict, mean_mse, mean_r = model.group_fit(win_interactions)
    logging.info("Difficulty curve fit MSE: {:.3f}, R^2: {:.3f} [{:<.2f} s]".
                 format(mean_mse, mean_r,
                        time.time() - t0) + os.linesep)
...
def add_args(parser):
    ...
    parser.add_argument('--file_prefix',
                        type=str,
                        default='',
                        help="Prefix of save parameter file.")
    parser.add_argument('--Neighbor_num',
                        type=int,
                        default=5,
...
if __name__ == "__main__":
    ...
    save_filename = os.path.join(args.save_path,
                                 args.file_prefix + "_difficulty_flow.json")
