...
def add_args(parser):
    ...
    parser.add_argument(
        '--cox_feature_path',
        type=str,
        default='cox_dataset/fold-XXX/D-Cox-Time/',
        help="Path for cox results, where 'XXX' will be replace by fold num.")
...
class Features_generator:
    ...
    def data_load(self):
        ...
        self.all_features = pd.read_csv(
            os.path.join(data_path, "day_features.csv"))
        self.user_label = pd.read_csv(os.path.join(data_path, "labels.csv"))
        save_param = os.path.join(data_path,
                                  "Difficulty_Flow/0_difficulty_flow.json")
        ...
        self.flow_features = pd.DataFrame(np.array([user_ids, a, b]).T,
                                          columns=["user_id", "a", "b"])

        self.beta_diff, self.beta_scale = [], []
        for fold in range(self.fold_num):
            save_beta = os.path.join(
                self.cox_feature_path.replace("XXX", str(2)), "diff_beta.npy")
            save_scale = os.path.join(
                self.cox_feature_path.replace("XXX", str(2)), "diff_scale.npy")
    ...
    def sample_construct(self):
        ...
        for i, u in (self.user_label.iterrows()):
            part_features = self.all_features.loc[
                (self.all_features.user_id == u.user_id)
        ...
        sample_features["retry_time"] = sample_features.retry_time.astype(
            float)
        sample_features[
            "global_retrytime"] = sample_features.global_retrytime.astype(
                float)
    ...
    def basic_features(self):
        ...
        self.sample_features["interval_length"] = self.sample_features[
            "end_day"] - self.sample_features["start_day"]
        day_features = self.sample_features.groupby("user_id").agg({
            "duration":
            "sum",
            "global_retrytime":
            "count",
        ...
        for feature in ["duration", "play_num", "level_num", "session_num"]:
            day_features[feature] = day_features[feature].astype(
                float) / day_features["interval_length"]
        day_features.rename(columns={"global_retrytime": "day_login"},
                            inplace=True)
        day_features["frequency"] = day_features["day_login"].astype(
            float) / day_features["interval_length"]
        # first purchase interval，last purchase interval
        first_purchase = self.sample_features.loc[
            (self.sample_features.gold_amount > 0) |
            (self.sample_features.coin_amount > 0)].groupby("user_id").head(1)
        last_purchase = self.sample_features.loc[
            (self.sample_features.gold_amount > 0) |
            (self.sample_features.coin_amount > 0)].groupby("user_id").tail(1)
        first_purchase.rename(columns={"day_since_register": "first_interval"},
                              inplace=True)
        first_purchase[
            "first_interval"] = first_purchase["first_interval"].astype(
                float) - first_purchase["start_day"].astype(float)
        last_purchase["last_interval"] = last_purchase["end_day"].astype(
            float) - last_purchase["day_since_register"].astype(float)

        day_features = day_features.merge(
            first_purchase.loc[:, ["user_id", "first_interval"]],
            on="user_id",
            how="left")
        day_features = day_features.merge(
            last_purchase.loc[:, ["user_id", "last_interval"]],
            on="user_id",
            how="left")
        ...
        day_features.to_csv(os.path.join(self.save_path, "feature_data.csv"),
                            index=False)
    ...
    def diff_features(self):
        ...
        diff_features = self.sample_features.groupby("user_id").agg({
            "retry_time": ["mean", "var"],
            "global_retrytime": ["mean", "var"]
        }).reset_index()
        diff_features.columns = [
            "user_id", "retry_time", "var_retry", "global_retrytime",
            "var_globalretry"
        ]
        diff_lastday_features = self.sample_features.groupby("user_id").tail(
            1).loc[:, ["user_id", "retry_time", "global_retrytime"]]
        diff_lastday_features.rename(columns={
            "retry_time": "lastday_retry",
            "global_retrytime": "lastday_globalretry"
        },
                                     inplace=True)
        diff_features = diff_features.merge(diff_lastday_features,
                                            on=["user_id"],
                                            how="left")
        diff_features = diff_features.merge(self.day_features,
                                            on=["user_id"],
                                            how="left")
        ...
        sample_features_diff = sample_features_diff.merge(self.flow_features,
                                                          on="user_id",
                                                          how="left")
        ...
        d_features = sample_features_diff.groupby(["user_id"]).agg({
            "PPD": ["mean", "var"]
        }).reset_index()
        d_features.columns = ["user_id", "mean_d", "var_d"]
        d_last_features = sample_features_diff.groupby(["user_id"]).tail(1)

        d_features = d_features.merge(d_last_features.loc[:,
                                                          ["user_id", "PPD"]],
                                      on="user_id",
                                      how="left")
        d_features.rename(columns={"PPD": "last_d"}, inplace=True)
        diff_features = d_features.merge(diff_features,
                                         on="user_id",
                                         how="right")
        diff_features.fillna(0, inplace=True)

        logging.info("Saving difficulty features...")
        diff_features.to_csv(os.path.join(self.save_path,
                                          "feature_data_diff.csv"),
                             index=False)
    ...
    def pd_features(self):
        ...
        for fold in range(self.fold_num):
            ...
            for i, row in (self.sample_features_diff.iterrows()):
                d = row.retry_time - row.a * row.global_retrytime - row.b
                cox = self.beta_diff[fold][int(row.day_since_register),
                                           Range(d)]
                cox_value.append(cox)
            self.sample_features_diff["beta"] = cox_value

            beta_features = self.sample_features_diff.groupby(["user_id"]).agg(
                {
                    "beta": ["mean", "var"]
                }).reset_index()
            beta_features.columns = ["user_id", "mean_beta", "var_beta"]
            beta_last_features = self.sample_features_diff.groupby(["user_id"
                                                                    ]).tail(1)
            beta_features = beta_features.merge(
                beta_last_features.loc[:, ["user_id", "beta"]],
                on="user_id",
                how="left")
            beta_features.rename(columns={"beta": "last_beta"}, inplace=True)
            beta_features = beta_features.merge(self.diff_features,
                                                on=["user_id"],
                                                how="right")
            beta_features.fillna(0, inplace=True)
            beta_features.to_csv(os.path.join(
                self.save_path, "feature_data_all_fold-%d.csv" % (fold + 1)),
                                 index=False)
...
if __name__ == "__main__":
    ...
    if args.log_file == '':
        args.log_file = 'logs/{}/{}.txt'.format("feature_based_data",
                                                args.data_path + ".log")
