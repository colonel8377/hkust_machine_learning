...
class Data_loader:
    ...
    def load_data(self):
        self.feature_df = pd.read_csv(
            os.path.join(self.datapath, self.feature_file))
        label = pd.read_csv(os.path.join(self.datapath, self.label_file))
        self.label = label[["user_id", "label"]]
        self.feature_df = self.feature_df.drop(columns=["label"]).merge(
            self.label, on=["user_id"])
    ...
    def generate_data(self):
        ...
        train_df = self.feature_df.loc[self.feature_df.user_id.isin(
            self.uid_list[0])]
        val_df = self.feature_df.loc[self.feature_df.user_id.isin(
            self.uid_list[1])]
...
class FM_Data_loader(Data_loader):
    def __init__(self,
        ...
        super(FM_Data_loader, self).__init__(datapath=datapath,
    ...
    def feature_processing(self):
        ...
        dense_features = self.feature_df.columns.drop([
            "user_id", "label", "interval_length", "first_interval",
            "last_interval"
        ]).tolist()
        self.feature_df[sparse_features] = self.feature_df[
            sparse_features].fillna('-1', )
        self.feature_df[dense_features] = self.feature_df[
            dense_features].fillna(0, )
        self.target = ['label']
        for feat in sparse_features:
            lbe = LabelEncoder()
            self.feature_df[feat] = lbe.fit_transform(self.feature_df[feat])
        mms = StandardScaler()
        self.feature_df[dense_features] = mms.fit_transform(
            self.feature_df[dense_features])
        fixlen_feature_columns = [
            SparseFeat(feat,
                       vocabulary_size=self.feature_df[feat].nunique(),
                       embedding_dim=4)
