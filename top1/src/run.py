...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_num_hidden_layers', type=int, default=6)
    ...
    args.data_path = os.path.join(args.data_path, 'workspaceblobstore', 'datasets', 'ad_trim')
    args.pretrained_model_path = os.path.join(args.pretrained_model_path, 'workspaceblobstore', 'datasets', 'ad_trim')
    base_path = args.data_path
    # 定义浮点数特征
    dense_features = ['user_id__size', 'user_id_ad_id_unique', 'user_id_creative_id_unique',
                      'user_id_advertiser_id_unique', 'user_id_industry_unique', 'user_id_product_id_unique',
                      'user_id_time_unique', 'user_id_click_times_sum', 'user_id_click_times_mean',
    ...
    args.pretrained_model_path = os.path.join(args.pretrained_model_path, 'BERT', 'bert-base')
    ...
    args.output_dir = "./output/saved_models/index_{}".format(args.index)
    ...
    for x in args.text_features:
        if x[0] not in args.embeddings_tables:
            try:
                args.embeddings_tables[x[0]] = gensim.models.Word2Vec.load_word2vec_format(x[0], binary=False)
            except:
    ...
    # 读取数据
    train_df = pd.read_pickle(os.path.join(args.data_path, 'train_user.pkl'))
    train_df['label'] = train_df['age'] * 2 + train_df['gender']
    test_df = pd.read_pickle(os.path.join(args.data_path, 'test_user.pkl'))
